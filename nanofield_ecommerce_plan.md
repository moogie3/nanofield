...
*Last updated: September 7, 2026 (v7) — RajaOngkir Phase 1 code complete (provider + variant weights), J&T ruled out on Starter; see `whole.md` for full project state*
...
- [ ] Still pending: real catalog import (879 products / 990 SKUs + un-uploaded remainder) from the Shopee mass-update Excel export — demo seed is 12 SKUs only and must not be confused with the real import. NOTE (Sep 6): the live catalog currently contains `Loadtest ... MOD-90xxx` placeholder products (EUR prices) plus deleted-fashion leftovers served from stale cache — clean these out before/with the real import, and restart the storefront dev server once to flush pre-ISR cache entries
- [ ] Still pending: resolve Section 6 data issues (1 SKU collision, 6 price mismatches, 2 duplicates, 38+6 platform gaps)
- [x] RajaOngkir Phase 1 code (Sep 7–8): `src/modules/rajaongkir-fulfillment/` provider (live V2 quotes JNE REG + J&T EZ, flat-fallback, lint + tsc clean, boot verified) + engine `Berat`→variant-weight parsing; `RAJAONGKIR_*` env in `.env`/`.env.template`. Live-verified Sep 8: key works, origin pinned 19363 (Pasar Jambi 36133), Jambi→Jakarta 1kg = JNE REG Rp 26.000 + J&T EZ Rp 22.000; OKE/YES dropped (absent ex-Jambi). Still pending: admin shipping-option setup (fulfillment set + Indonesia zone + 2 calculated options + pickup) and live checkout verification. Services after that are admin-managed via `/app/rajaongkir-services` (add/disable/delete live, no deploy) — console never needed in production
- [ ] Still pending: Midtrans provider, Postgres full-text search (replaces Meilisearch — no new infra), R2, hosting decision, PSE check (see Section 7)
...
- [ ] Midtrans provider, Postgres full-text search (replaces Meilisearch), R2 (all decided, none started); RajaOngkir provider code DONE — remaining: admin setup (fulfillment set + Indonesia zone + 4 calculated options + pickup) and live verification
