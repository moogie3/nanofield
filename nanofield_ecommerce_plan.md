...
*Last updated: September 9, 2026 (v8) — v7 record preserved below; amendments: Postgres full-text/trigram search shipped Sep 8 (Meilisearch dropped, no new infra); shipping state per `whole.md` (fulfillment set + Indonesia zone + 3 calculated options JNE REG / J&T EZ / JNE City Courier verified on lanes; OKE/YES dropped; checkout verification still pending); see `whole.md` for full project state*

> **Decision history, for context:** v1 recommended Medusa. v2 briefly moved to Payload CMS on the reasoning that checkout was being fully custom-built anyway, so Medusa's commerce engine bought less than it looked like. v3 reverses back to Medusa, because the store's stated goal is B2C-first with **B2B as a near-future possibility, not a current requirement** — and Medusa's B2B modules (customer groups, price lists, company accounts, quotes) are dormant/free until used, while Payload has none of that scaffolding and would require building B2B commerce logic from scratch later. v4 renamed to Nanofield, confirmed backend decision unchanged. v5 synced to what was committed on `main` — no decision change. v6 (this version): no decision change; records the Shopee import pipeline, admin branding, storefront UX work, and catalog caching/filter fixes built since v5. v7: RajaOngkir Phase 1 code complete — custom fulfillment provider (JNE REG/OKE/YES + J&T ekonomis, live quotes with flat fallback) + Shopee `Berat` → variant weight plumbing. Decision changes: **integration targets the V2 Komerce API, not legacy v1** (free tier incl. J&T, subdistrict ids, sandbox for testing); **J&T is back in** (V2 free tier covers it — the Pro-only limit was legacy-v1-only). Sep 7 outage postmortem: dead backend → storefront 500s; provider must be injected via fulfillment `options.providers`, never top-level `modules[]`.
...
- [x] Shopee importer pipeline built (backend): shared engine (`src/api/admin/shopee-imports/engine.ts`), Admin API routes (`preview`, `execute`, `jobs`, `jobs/[id]`), in-memory job store, multer middleware for `sales`/`basic`/`media` workbooks, thin CLI client (`import-shopee.mjs`). Dry-run verified against the live export (880 products, 0 errors). Admin UI: `/app/ecomm-import` route (E-comm Import, `ArrowUpTray` icon) + `/app/overview` Nanofield home route
- [x] Admin branding suite: login circuit backdrop (22-trace board, 14 component glyph types, 39+ spinning floaters, blinking vias), topbar badge, sidebar avatar blobs, favicon, Outfit+Manrope fonts, static-copy scrubber (hides Docs/Changelog, rewrites Medusa strings). Product datasheet widget (`product.details.side`: toggle + URL + MPN). Store renamed to "Nanofield" via API
- [x] Storefront UX pass: hover preview (viewport-clamped, mobile-hidden), default product image `size` system (transparent, no bg square), optimistic `QuantityStepper` (cart page + dropdown), category filter "All products" reset row, shared datasheet util (`no_datasheet` opt-out → direct URL → search fallback) + `DatasheetButton`, image gallery carousel (arrows/dots/counter), unified hero schematic scene (see below)
- [x] Hero redesigned as one connected schematic (`HeroSchematic` in `modules/home/components/hero/index.tsx`): power block → MCU → ADC/sensor → motor driver → comms → relay output, all nets orthogonal with junction dots, inline diode/Zener/LED/MOSFET/NPN/relay/switch symbols; draggable overlay symbols kept as separate layer
- [x] Catalog filter/cache reliability (Sep 6): "All products" reset now `router.refresh()`es + grid remounts via filter-signature `key` on the `<Suspense>` boundary; empty-filter result renders an explicit "No products match" panel with a "Show all products" link; products/categories fetches changed from indefinite `force-cache` to 5-min ISR (`revalidate: 300`) so admin deletes stop haunting the storefront; `.vscode/settings.json` silences editor-only `@tailwind`/`@apply` warnings
- [ ] Still pending: real catalog import (879 products / 990 SKUs + un-uploaded remainder) from the Shopee mass-update Excel export — demo seed is 12 SKUs only and must not be confused with the real import. Import procedure (backend setup order, Preview sign-off, Execute) is defined in `STARTING_MANUAL.md` Parts E–F. NOTE (Sep 6): the live catalog currently contains `Loadtest ... MOD-90xxx` placeholder products (EUR prices) plus deleted-fashion leftovers served from stale cache — clean these out before/with the real import, and restart the storefront dev server once to flush pre-ISR cache entries
- [ ] Still pending: resolve Section 6 data issues (1 SKU collision, 6 price mismatches, 2 duplicates, 38+6 platform gaps)
- [x] RajaOngkir Phase 1 code (Sep 7–8): `src/modules/rajaongkir-fulfillment/` provider (live V2 quotes JNE REG + J&T EZ + JNE City Courier, flat-fallback, lint + tsc clean, boot verified) + engine `Berat`→variant-weight parsing; `RAJAONGKIR_*` env in `.env`/`.env.template`. Live-verified Sep 8: key works, origin pinned 19363 (Pasar Jambi 36133), Jambi→Jakarta 1kg = JNE REG Rp 26.000 + J&T EZ Rp 22.000, intra-Jambi JNE CTC Rp 10.000; OKE/YES dropped (absent ex-Jambi). Admin shipping setup complete per `whole.md` (fulfillment set + Indonesia zone + 3 calculated options); reproducible after a fresh clone via `apps/backend/scripts/seed-nanofield-shipping.mjs` or manually per `STARTING_MANUAL.md` Part F. Still pending: live checkout verification. Services after that are admin-managed via `/app/rajaongkir-services` (add/disable/delete live, no deploy) — console never needed in production
- [ ] Still pending: Midtrans provider, R2, hosting decision, PSE check (see Section 7). Postgres full-text/trigram search shipped Sep 8 (Meilisearch dropped — no new infra, no re-setup required).
...
| Search | Postgres full-text + trigram (Meilisearch evaluated, dropped Sep 8) | No new infrastructure; typo-tolerant part-number search verified live — matters for part-number search and mixed ID/EN product names |
...
- Postgres full-text + trigram search in front of the catalog — sub-10ms search/filter at this catalog size
...
- **Not all inventory is on Shopee/TikTok yet** — user has confirmed there's additional stock never uploaded to either platform. Scale beyond 990 SKUs is still unknown; doesn't change the architecture (Postgres doesn't care whether it's 990 or 5,000+), but should be factored into the import plan once available.
...
- [ ] Midtrans provider, R2 (decided, none started); RajaOngkir provider code DONE and admin setup complete per `whole.md` (fulfillment set + Indonesia zone + 3 calculated options JNE REG / J&T EZ / JNE City Courier); remaining: live checkout verification (see `STARTING_MANUAL.md` Part H)
...
- [ ] Housekeeping: consider moving the one-off `probe-admin.mjs` / `seed-semiconductors.mjs` scripts into a documented `apps/backend/scripts/` folder with usage in `README`. `whole.md` (whole-project state doc) is new as of v6 — keep it updated alongside this plan after each work session
