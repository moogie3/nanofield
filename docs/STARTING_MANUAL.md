...
- **Notification providers:** the `feed` channel through the local provider is connected — it powers the administration bell (import completion and failure, order placement and cancellation, team changes) AND the logged-in customer navbar bell (order confirmed/shipped/delivered/canceled, return requested/received plus claims/exchanges, keyed by customer email via `GET /store/notifications`). A nightly retention job (`notification-retention`, `FEED_RETENTION_DAYS` default 90) deletes `feed` rows past the cutoff. Customer email rides `resend-notification` (production, `RESEND_API_KEY` + verified `RESEND_FROM`) or `mailtrap-notification` sandbox (dev, `MAILTRAP_HOST/PORT/USER/PASS` SMTP credentials + `MAILTRAP_FROM`); exactly one registers, chosen by which credentials exist. Email templates cover order confirmation/shipped/delivered/canceled plus return requested/received/update. No email provider → customer sends skip silently and the admin "notify customer" checkbox does nothing.
...
- **Authentic catalog:** administration → `/app/ecomm-import`. Upload all four workbooks (sales, basic, media, and **ship/Informasi Pengiriman** — the ship file carries per-variation `Berat` weights; without it every variant falls back to 500 g and light items are over-quoted). **Preview** is always to be executed first and constitutes the formal Gate 1 sign-off (Part E, item G1.5): **Execute** follows only upon a signed-off Preview — location resolving to Pasar Jambi, genuine Stok figures, detected weight header. Preview additionally reports catalog-consistency badges: variation labels cleaned, categories remapped to canonical, specs derived, products flagged with datasheet (full contract in `docs/catalog-consistency-phases.md`). Categories resolve through the canonical allowlist — genuinely new Shopee leaves appear under `categoriesToCreate` for deliberate triage instead of scattering; re-imports match by `shopee_product_id` and add zero new option values on identical files.
...
| E-comm Import | `/app/ecomm-import` | Shopee Excel import pipeline. Forms: four workbook upload fields (sales, basic, media, ship/Informasi Pengiriman); **Preview** action (validates without writing; lists skipped rows with identifier and reason; reports the detected weight column plus catalog-consistency badges for variation cleanup, category remapping, and spec/datasheet coverage); **Execute** action (writes the catalog); job list with per-job detail (counts, errors). A **Datasheet auto-link** card below fills empty MPNs from titles, links curated-map PDFs, and flags `has_datasheet` across the catalog in one click (dry-run report first, `usePrompt` confirm on apply; metadata only). Purpose: the sole entry point of the authentic catalog. |
...
| Announcements | `/app/announcements` | Bell broadcasts plus storefront banners, in four sections. **Broadcast** form (title/required, description, storefront-path link, optional "banner until" date): posts one feed record to every logged-in customer bell (visible on the next 60s poll; also lands in the admin bell). **Image banner** form (title, link, JPEG/PNG/WebP file ≤5MB, shown-through date): publishes the fixed homepage/store banner (files land in the backend `/static` volume; only the relative path is stored). **Announcement strip** form: text strip with no bell involved. **Banners** list: every row shows a live preview (image thumb or strip mock) with Publish/Unpublish toggle and dates — unpublish hides immediately, expired rows stay as history, nothing is ever deleted. **Past broadcasts** list: inline Edit (applies going forward only) and confirm-gated Delete (bell record only; banner twins are separate rows). Every mutation toasts success; failures show inline. Purpose: all customer-facing messaging from one page. |
...
- **A wedged backend holds the port with nothing listening** (September 2026 incident: login `fetch failed`, broken `/static` images, empty customer bell, zero red in any log). The fix is a full kill of stray processes (`Get-Process node | Stop-Process -Force`) before restarting — a plain Ctrl+C plus restart can leave the stray in place.
- **Windows file paths must never reach stored URLs** (September 2026 incident: banner `image_url` persisted with backslashes from `path.join`). Persist forward slashes on write and normalize defensively at read.
- **`medusa develop` applies pending module migrations on boot** (proven September 2026) — but the admin UI hot-reloads on its own, so new admin pages render against a stale API until the backend itself is restarted; that combination looks exactly like a backend bug.
